...
export class DeclarativeShadowElement extends HTMLElement {
    connectedCallback() {
        if (!this.shadowRoot) {
            ...
            this.attachShadow({
                mode: "open"
            }).append(template.content.cloneNode(!0))
        }
    }
}
export class Component extends DeclarativeShadowElement {
    ...
    connectedCallback() {
        super.connectedCallback(), registerEventListeners(), this.#updateRefs(), requestIdleCallback(() => {
            for (const root of this.roots) this.#mutationObserver.observe(root, {
                childList: !0,
                subtree: !0,
                attributes: !0,
                attributeFilter: ["ref"],
    ...
    mutationObserver = new MutationObserver(mutations => {
        mutations.some(m => m.type === "attributes" && this.#isDescendant(m.target) || m.type === "childList" && [...m.addedNodes, ...m.removedNodes].some(this.#isDescendant)) && this.#updateRefs()
    });#
...
function getClosestComponent(node) {
    if (!node) return null;
    if (node instanceof Component || node instanceof HTMLElement && node.tagName.toLowerCase().endsWith("-component")) return node;
...
function registerEventListeners() {
    ...
    for (const eventName of [...events, ...expensiveEvents]) {
        ...
        document.addEventListener(eventName, event => {
            ...
            if (!element) return;
            const proxiedEvent = event.target !== element ? new Proxy(event, {
                    get(target, property) {
            ...
            const matches = value.match(/([\/\?][^\/\?]+)([\/\?][^\/\?]+)$/),
                ...
                instance = selector ? selector.startsWith("#") ? document.querySelector(selector) : element.closest(selector) : getClosestComponent(element);
    ...
    function getElement(event) {
        ...
        if (target instanceof Element) return target.hasAttribute(`on:${event.type}`) ? target : expensiveEvents.includes(event.type) ? null : event.bubbles || shouldBubble.includes(event.type) ? target.closest(`[on\\:${event.type}]`) : null
    }
}
...
//# sourceMappingURL=/cdn/shop/t/38/assets/component.js.map?v=184240237101443119871768999425