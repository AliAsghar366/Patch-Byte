import {
    buildSectionSelector,
    normalizeSectionId,
    sectionRenderer
} from "@theme/section-renderer";
import {
    requestIdleCallback,
    onDocumentReady
} from "@theme/utilities";
async function hydrateSection(sectionId, url) {
    const normalizedId = normalizeSectionId(sectionId),
        section = document.getElementById(buildSectionSelector(normalizedId));
    !section || section.dataset.hydrated === "true" || (await sectionRenderer.renderSection(normalizedId, {
        cache: !1,
        url
    }), section.dataset.hydrated = "true")
}
export async function hydrate(sectionId, url) {
    onDocumentReady(() => {
        requestIdleCallback(() => hydrateSection(sectionId, url))
    })
}
//# sourceMappingURL=/cdn/shop/t/38/assets/section-hydration.js.map?v=177651605160027701221768999423